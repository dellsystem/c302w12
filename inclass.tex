...
\begin{enumerate}
    \item Are \emph{all} substitution instances of \verb~e~ well-typed? \\
        If e has some type T and containts some type variables $\alpha_1, \ldots, \alpha_n$, then e has type \verb~[T1/'a1 ... Tn/'an] T~ for every $T_1, \ldots T_n$?
    \item Does there exist some substitution instatnce such that e has type T?
\end{enumerate}
...
Will \verb~fn x => x+1~ have type \verb~'b->bool~? No. there is instantatiation for \verb~'b~.
...
What is a contstraint? For example, \verb~T=bool~.
...
'a1 = 'a4 -> int
'a2 = 'a3 -> 'a4, 'a2 = 'a3 -> int (intutitively)
'a1 = 'a4 -> int
.: 'a4 = int, 'a1 = int -> int
No constraint on 'a3
...
Does there exist an insubstantiation for 'a1 such that
    'a1 = ('a1 -> int) -> int ?
...
Question: Does there exist an insubstatiation such that all constraints are true?
...
(* this will typecheck in SML, but would not type check in our definition so far,
 ...
 * it can reuse the type over multiple types by saying "forall 'a2:('a2->'a2)->'a2->'a"
 *)
\end{verbatim}
...
In ML:
(generalize -- abstract over the free variables and quanitify over them, so we can reuse them as often as we'd like)
\begin{verbatim}
...
In example 2, \verb~(forall 'a, 'a -> 'a) -> int~ -- full polymorphism. In ML, we have parametric polymporphism, where all quantification over type variables is at the outside. Some languages are slowly moving towards allowing full polymorphism.
...
\textbf{Disadantages of not writing type annotations:}
\begin{itemize}
    ...
    \item Hindley-Milner type inference does not scale to richer types such as subtyping, full polymorphism, and dependent types. 
    \item Hindley-Milner type inference is complicated and difficult to implement (unification is needed).
\end{itemize}

Basic idea behind bi-directional type checking -- instead of infering a type:
    \[
        \Gamma \vbar e \Rightarrow \tau
    \]
$\forall e$ where $\tau$ is the output, we'll use two judgements (functions), ``check" and ``synthesize". First, check $\Gamma \vbar e \Leftarrow \tau$ for inputs $\Gamma, e, \tau$ (check that expression $e$ has type $\tau$ under the assumptions $\Gamma$). Then, synthesize $\Gamma \vbar e \Rightarrow \tau$ (synthesize a type $\tau$ for expression $e$ under the assumptions $\Gamma$ for \emph{some} $e$). Synthesize basically means infer.
...
Type variables:
\[
    \infer{\Gamma_i x : \tau \vbar x => \tau}{}
\]

Functions
\[
    \infer{\Gamma \vbar \verb~fn x => e~ \Rightarrow \tau_1 \rightarrow \tau_2}{
        \tau_i x : \tau_1 \vbar e \Leftarrow \tau_2
    }
\]

\[
    \infer{\Gamma \vbar e_1 e_2}{
        \Gamma \vbar e_1 \Rightarrow \tau_2 \rightarrow \tau
        &
        \Gamma \vbar e_2 \Leftarrow \tau_2
    }
\]
...
Conversion:
\[
    \infer{\Gamma \vbar e \Leftarrow \tau}{
        \Gamma \vbar e \Rightarrow \tau^{\prime}
        &
        \tau^{\prime} = \tau
    }
\]
...
Example:
\[
    \infer[by conversion]{\vbar 3 \Leftarrow \verb~int~}{
        \vbar 3 \Rightarrow \verb~int~
        &
        \verb~int = int~
    }
\]
